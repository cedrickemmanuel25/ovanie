...
@section('title', 'Connexion Client ou Vendeur')

@section('content')
<div class="bg-white rounded-2xl shadow-lg p-5 lg:p-6 w-full" x-data="{ loading: false, showPassword: false }">
    <!-- Logo -->
    ...
    <div class="text-center mb-5">
        <h1 class="font-bold text-2xl text-[#1E3A5F] mb-1 leading-tight">Connexion Client<br>ou Vendeur</h1>
        <p class="text-gray-500 text-sm">Cette page est réservée aux clients et aux vendeurs OVANIE</p>
    </div>


    @if (session('info'))
        <div class="mb-4 p-3 rounded-xl bg-blue-50 border border-blue-100 text-blue-700 text-xs text-center">
            {{ session('info') }}
        </div>
    @endif

    @if (session('error'))
        <div class="mb-4 p-3 rounded-xl bg-red-50 border border-red-100 text-red-600 text-xs text-center">
            {{ session('error') }}
        </div>
    @endif
    ...
    <form method="POST" action="{{ route('login.web') }}" @submit="loading = true">
        @csrf

        <!-- Email ou téléphone -->
        <div class="mb-4">
            <label for="identifier" class="block font-semibold text-gray-800 text-[13px] mb-1">Email ou téléphone</label>
            <div class="relative text-gray-400 focus-within:text-blue-500">
                ...
                <input id="identifier" type="text" name="email" value="{{ old('email') }}" required autofocus
        ...
        <div class="flex items-center justify-between mb-5">
            <label class="flex items-center cursor-pointer">
                <input type="checkbox" name="remember" class="w-4 h-4 text-[#2563EB] bg-white border-gray-300 rounded focus:ring-blue-500">
                <span class="ml-2 text-xs text-gray-600">Se souvenir de moi</span>
            </label>
            @if (Route::has('password.request'))
                <a href="{{ route('password.request') }}" class="text-xs text-[#2563EB] font-medium hover:underline">Mot de passe oublié ?</a>
            @endif
        </div>

        <!-- Bouton Submit -->
        <button type="submit" :disabled="loading" class="w-full bg-[#2563EB] hover:bg-blue-700 text-white font-semibold py-3 rounded-xl transition-colors flex items-center justify-center space-x-2 disabled:opacity-70 text-sm">
    ...
    <div class="relative my-5">
        ...
        <div class="relative flex justify-center text-xs"><span class="px-3 bg-white text-gray-400">Ou connectez-vous avec</span></div>
    </div>
    ...
    <div class="text-center mb-4">
        <p class="text-gray-500 text-xs">
            ...
            <a href="{{ route('register') }}" class="text-[#2563EB] font-bold hover:underline inline-flex items-center">
