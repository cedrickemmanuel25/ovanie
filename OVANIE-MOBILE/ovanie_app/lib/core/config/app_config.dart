...
/// Configuration d'environnement de l'application OVANIE Client.
///
/// En développement local, privilégier les scripts du dossier LOCAL-DEV qui
/// injectent automatiquement l'adresse IP LAN du PC avec --dart-define.
///
...
class AppConfig {
  ...
  static String _defaultLocalApiBase() {
    ...
    if (defaultTargetPlatform == TargetPlatform.android) {
      // Android Emulator uniquement. Sur un téléphone physique, utiliser
      // LOCAL-DEV/run-app.ps1 qui injecte automatiquement l'IP LAN du PC.
  ...
  static String normalizeMediaUrl(String? rawUrl) {
    ...
    if (value.startsWith('//')) {
      final scheme = Uri.tryParse(origin)?.scheme ?? 'http';
      return '$scheme:$value';
    }
